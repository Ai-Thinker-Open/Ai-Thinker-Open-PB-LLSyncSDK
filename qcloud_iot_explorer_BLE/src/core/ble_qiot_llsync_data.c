...
static ble_qiot_ret_status_t ble_lldata_property_data_handle(bool is_request, const char *in_buf, int buf_len)
{
    ...
    qcloud_property_down(in_buf, buf_len);
#else
...
#ifdef AITHINKER_AT_MODIFY 
ble_qiot_ret_status_t aithinker_ble_user_property_report_data(const char *data, uint16_t len)
{
    return ble_event_notify(BLE_QIOT_EVENT_UP_PROPERTY_REPORT, NULL, 0, (const char *)data, len);
}
#endif // AITHINKER_AT_MODIFY
...
#if AITHINKER_AT_MODIFY
    qcloud_action_down(action_id, in_buf, len);
    // please call action reply function to post action execute result 
#else
    while (parse_len < len) {
...
#if AITHINKER_AT_MODIFY
ble_qiot_ret_status_t aithinker_ble_lldata_action_reply_handle(uint8_t action_id, uint8_t result, const char *in_buf, int len)
{
    uint8_t header_buf[2] = {0};
    ...
    if (1 == result) {
        return ble_event_notify(BLE_QIOT_EVENT_UP_ACTION_REPLY, header_buf, sizeof(header_buf), (const char *)in_buf,
                                len);
    } else {
        header_buf[0] = BLE_QIOT_REPLY_FAIL;
        return ble_event_notify(BLE_QIOT_EVENT_UP_ACTION_REPLY, header_buf, sizeof(header_buf), (const char *)&in_buf,
