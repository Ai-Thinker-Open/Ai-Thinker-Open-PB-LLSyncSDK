...
static int ble_property_work_mode_set(const char *data, uint16_t len)
{
	int tmp_enum = 0;
	memcpy(&tmp_enum, data, sizeof(uint16_t));
	tmp_enum = NTOHL(tmp_enum);
	ble_qiot_log_d("set id work_mode int value %d", tmp_enum);
	return 0;
}

static int ble_property_work_mode_get( char *data, uint16_t len)
{
	uint16_t tmp_enum = 0;
	tmp_enum = HTONS(tmp_enum);
	memcpy(data, &tmp_enum, sizeof(uint16_t));
	ble_qiot_log_d("get id work_mode int value %d", 1234);
	return sizeof(uint16_t);
}

static int ble_property_hsv_color_hue_set(const char *data, uint16_t len)
{
	float tmp_float = 0;
	memcpy(&tmp_float, data, sizeof(float));
	ble_qiot_log_d("set id hue float value %f", tmp_float);
	return 0;
}

static int ble_property_hsv_color_hue_get( char *data, uint16_t len)
{
	float tmp_float = 1.23456;
	memcpy(data, &tmp_float, sizeof(float));
	ble_qiot_log_d("get id hue float value %f", tmp_float);
	return sizeof(float);
}

static int ble_property_hsv_color_saturation_set(const char *data, uint16_t len)
{
	float tmp_float = 0;
	memcpy(&tmp_float, data, sizeof(float));
	ble_qiot_log_d("set id saturation float value %f", tmp_float);
	return 0;
}

static int ble_property_hsv_color_saturation_get( char *data, uint16_t len)
{
	float tmp_float = 1.23456;
	memcpy(data, &tmp_float, sizeof(float));
	ble_qiot_log_d("get id saturation float value %f", tmp_float);
	return sizeof(float);
}

static int ble_property_hsv_color_value_set(const char *data, uint16_t len)
{
	float tmp_float = 0;
	memcpy(&tmp_float, data, sizeof(float));
	ble_qiot_log_d("set id value float value %f", tmp_float);
	return 0;
}

static int ble_property_hsv_color_value_get( char *data, uint16_t len)
{
	float tmp_float = 1.23456;
	memcpy(data, &tmp_float, sizeof(float));
	ble_qiot_log_d("get id value float value %f", tmp_float);
	return sizeof(float);
}

static ble_property_t sg_ble_hsv_color_property_array[3] = {
	{(property_set_cb)ble_property_hsv_color_hue_set, (property_get_cb)ble_property_hsv_color_hue_get, 0, BLE_QIOT_DATA_TYPE_FLOAT, 1},
	{(property_set_cb)ble_property_hsv_color_saturation_set, (property_get_cb)ble_property_hsv_color_saturation_get, 0, BLE_QIOT_DATA_TYPE_FLOAT, 1},
	{(property_set_cb)ble_property_hsv_color_value_set, (property_get_cb)ble_property_hsv_color_value_get, 0, BLE_QIOT_DATA_TYPE_FLOAT, 1},
};

static int ble_property_hsv_color_set(const char *data, uint16_t len)
{
	return ble_user_property_struct_handle(data, len, sg_ble_hsv_color_property_array, 3);
}

static int ble_property_hsv_color_get(char *data, uint16_t len)
{
	return ble_user_property_struct_get_data(data, len, sg_ble_hsv_color_property_array, 3);
}
...
static int ble_property_scene_set(const char *data, uint16_t len)
{
	int tmp_enum = 0;
	memcpy(&tmp_enum, data, sizeof(uint16_t));
	tmp_enum = NTOHL(tmp_enum);
	ble_qiot_log_d("set id scene int value %d", tmp_enum);
	return 0;
}

static int ble_property_scene_get( char *data, uint16_t len)
{
	uint16_t tmp_enum = 0;
	tmp_enum = HTONS(tmp_enum);
	memcpy(data, &tmp_enum, sizeof(uint16_t));
	ble_qiot_log_d("get id scene int value %d", 1234);
	return sizeof(uint16_t);
}

static int ble_property_color_value_set(const char *data, uint16_t len)
{
	char tmp_str[128] = "";//copy the actual length of the text
	memcpy(tmp_str, data, 1);
	ble_qiot_log_d("set id color_value string value %s", data);
	return 0;
}

static int ble_property_color_value_get( char *data, uint16_t len)
{
	char tmp_str[2] = "a";
	memcpy(data, tmp_str, strlen(tmp_str));
	ble_qiot_log_d("get id color_value string value %s", data);
	return strlen(tmp_str);
}

static int ble_property_count_down_set(const char *data, uint16_t len)
{
	int tmp_int = 0;
	memcpy(&tmp_int, data, sizeof(int));
	tmp_int = NTOHL(tmp_int);
	ble_qiot_log_d("set id count_down int value %d", tmp_int);
	return 0;
}

static int ble_property_count_down_get( char *data, uint16_t len)
{
	int tmp_int = 1;
	tmp_int = HTONL(tmp_int);
	memcpy(data, &tmp_int, sizeof(int));
	ble_qiot_log_d("get id count_down int value %d", 12345678);
	return sizeof(int);
}
...
ble_property_t sg_ble_property_array[9] = {
	{ble_property_power_switch_set, ble_property_power_switch_get, 0, BLE_QIOT_DATA_TYPE_BOOL, 1},
	{ble_property_work_mode_set, ble_property_work_mode_get, 0, BLE_QIOT_DATA_TYPE_ENUM, 1},
	{ble_property_hsv_color_set, ble_property_hsv_color_get, 0, BLE_QIOT_DATA_TYPE_STRUCT, 1},
	...
	{ble_property_scene_set, ble_property_scene_get, 0, BLE_QIOT_DATA_TYPE_ENUM, 1},
	{ble_property_color_value_set, ble_property_color_value_get, 0, BLE_QIOT_DATA_TYPE_STRING, 1},
	{ble_property_count_down_set, ble_property_count_down_get, 0, BLE_QIOT_DATA_TYPE_INT, 1},
